-- Sample 005: Partition Details Analysis Procedure
-- Source: MSSQLTips - Implementation of Sliding Window Partitioning
-- Category: Partitioning
-- Complexity: Complex
-- Features: System views, Window functions (LAG/LEAD), Partition metadata
...
BEGIN
    ...
    SELECT
        ...
        CASE
            ...
            THEN c.name + ' > ' + 
                 CAST(ISNULL(LAG(prv.value) OVER(
                     PARTITION BY pstats.object_id 
            ...
            ELSE c.name + ' >= ' + 
                 CAST(ISNULL(prv.value, 'Infinity') AS VARCHAR(100)) + 
                 ' and ' + c.name + ' < ' + 
                 CAST(ISNULL(LEAD(prv.value) OVER(
                     PARTITION BY pstats.object_id 
                     ORDER BY pstats.object_id, pstats.partition_number
                 ), 'Infinity') AS VARCHAR(100))
